...
def plot_loss(losses, filename: str = "loss_curve_gru.png"):
...
def plot_prediction(y_true: torch.Tensor,
                    y_pred: torch.Tensor,
                    filename: str = "prediction_gru.png"):
    ...
    plt.title("GRU Next-Step Prediction (Test)")
...
def plot_hidden(h_all: torch.Tensor,
                filename: str = "hidden_state_gru.png"):
    """
    h_all: (B, T, H)
    """
    h = h_all[0]        # (T, H)
    ...
    plt.figure(figsize = (8, 4))
    plt.plot(h_dim0)
    plt.title("GRU Hidden State (dim 0 of sample 0)")
